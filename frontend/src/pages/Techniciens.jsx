...
import isoWeek from 'dayjs/plugin/isoWeek';   // ✅ plugin requis pour lundi en 1er
import isSameOrBefore from 'dayjs/plugin/isSameOrBefore';

// ✅ Enregistrer les plugins AVANT tout usage
...
const ModalPlanning = ({ technicien, open, onClose }) => {
    ...
    const chargerInterventions = async () => {
        setLoading(true);
        try {
            const res = await api.get(`/interventions/?technicien_id=${technicien.id}`);
    ...
    // ✅ Calendrier aligné lundi→dimanche grâce à isoWeek
    const buildCalendar = () => {
        ...
        // On itère tant qu'on n'a pas dépassé la fin
        while (jour.isSameOrBefore(fin, 'day')) {
            const semaine = [];
            for (let d = 0; d < 7; d++) {
                semaine.push(jour);
                jour = jour.add(1, 'day');
    ...
    return (
        ...
        >
            ...
            <div style={{
                ...
                <Avatar size={52} style={{
                    background: '#FF8C00', fontSize: 22, fontWeight: 700,
                    flexShrink: 0, boxShadow: '0 0 0 3px rgba(255,140,0,0.3)'
                }}>
                ...
                <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: 13, marginTop: 2 }}>
                        <Tag color={couleurSpecialite[technicien?.specialite]} style={{ borderRadius: 4, fontSize: 11 }}>
                ...
                {[
                    ...
                    <div key={s.label} style={{
                        ...
                        borderRadius: 10, padding: '8px 16px', minWidth: 64
                    }}>
            ...
            <div style={{ padding: '20px 28px 28px' }}>
                {loading ? (
                    ...
                    <Empty description="Aucune intervention planifiée" style={{ padding: '40px 0' }} />
                ) : (
                    <>
                            ...
                            {vueMode === 'calendrier' && (
                                <Space>
                                    <Button size="small" icon={<LeftOutlined />}
                                        onClick={() => setMoisCourant(m => m.subtract(1, 'month'))}
                                    ...
                                    <Button size="small" icon={<RightOutlined />}
                                        onClick={() => setMoisCourant(m => m.add(1, 'month'))}
                                        style={{ borderRadius: 6 }} />
                                    <Button size="small"
                                        onClick={() => setMoisCourant(dayjs())}
                                        style={{
                                            borderRadius: 6, background: '#FF8C0015',
                                            borderColor: '#FF8C00', color: '#FF8C00', fontWeight: 600
                                        }}>
                                        Aujourd'hui
                                    </Button>
                                </Space>
                                ...
                                {buildCalendar().map((semaine, si) => (
                                    <div key={si} style={{
                                        ...
                                        {semaine.map((jour, ji) => {
                                            ...
                                            const estAuj    = dateStr === aujourd_hui;
                                            // isoWeekday : 6=Sam, 7=Dim → ji >= 5 dans la grille
                                            const weekend   = ji >= 5;
                                            ...
                                            return (
                                                <div key={ji} style={{
                                                    ...
                                                    <div style={{
                                                        fontWeight: estAuj ? 800 : 500,
                                                        fontSize: 12, marginBottom: 3,
                                                        color: estAuj ? '#FF8C00' : weekend ? '#fa541c' : '#333',
                                                    }}>
                                                        {jour.format('D')}
                                                    </div>

                                                    {intervs.map(i => (
                                                        <Tooltip key={i.id} title={
                                                            <div>
                                                                <div style={{ fontWeight: 700 }}>{i.numero}</div>
                            ...
                            <div style={{ maxHeight: 480, overflowY: 'auto', paddingRight: 4 }}>
                                <Timeline
                                    items={interventionsTriees.map(i => {
                                        ...
                                        const estAuj   = d.format('YYYY-MM-DD') === aujourd_hui;
                                        ...
                                        return {
                                            ...
                                            children: (
                                                <div
                                                    onClick={() => setDetailOuvert(i)}
                                                    style={{
                                                        background: estAuj ? '#FFF8F0' : '#fafafa',
                                                        border: estAuj ? '1px solid #FF8C0055' : '1px solid #f0f0f0',
                                                ...
                                                >
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                        <div>
                                                            <span style={{ color: '#FF8C00', fontWeight: 700, fontSize: 13 }}>{i.numero}</span>
                                                            <span style={{ color: '#333', fontWeight: 600, marginLeft: 8 }}>{i.client_nom}</span>
                                                        </div>
                                                        <div style={{ display: 'flex', gap: 6 }}>
                                                            <span style={{
                                                                fontSize: 10, fontWeight: 600, padding: '2px 7px',
                                                                borderRadius: 10, color: urg.color,
                                                                background: urg.bg, border: `1px solid ${urg.border}`
                                                            }}>
                                                                {i.urgence?.toUpperCase()}
                                                            </span>
                                                            <Tag color={couleurStatut[i.statut]} style={{ borderRadius: 6, fontSize: 10, margin: 0 }}>
                                                                {i.statut?.toUpperCase()}
                                                            </Tag>
                                                        </div>
                                                    </div>
                                                    <div style={{ display: 'flex', gap: 14, marginTop: 5, color: '#888', fontSize: 12 }}>
                                                        <span>
                                                            <ClockCircleOutlined style={{ marginRight: 4 }} />
                                                            <span style={{
                                                                color: estAuj ? '#FF8C00' : estPasse ? '#f5222d' : '#555',
                                                                fontWeight: estAuj ? 700 : 400
                                                            }}>
                                                                {d.format('ddd DD/MM/YYYY')} {d.format('HH:mm')}
                                                                {estAuj && (
                                                                    <span style={{
                                                                        marginLeft: 6, background: '#FF8C00',
                                                        ...
                                                        {i.duree_estimee && <span>⏱ {i.duree_estimee}h</span>}
            ...
            <Modal
                ...
                footer={[<Button key="close" onClick={() => setDetailOuvert(null)}>Fermer</Button>]}
                width={440}
                title={
                    <Space>
                        ...
                        {detailOuvert && (
                            <Tag color={couleurStatut[detailOuvert.statut]} style={{ borderRadius: 6 }}>
            ...
            >
                {detailOuvert && (
                    <>
                        <div style={{
                            background: '#FF8C0011', border: '1px solid #FF8C0033',
                            borderRadius: 10, padding: '14px 18px', marginBottom: 16,
                            display: 'flex', alignItems: 'center', gap: 12
                        }}>
                            <ClockCircleOutlined style={{ fontSize: 22, color: '#FF8C00' }} />
                            <div>
                                <div style={{ fontWeight: 700, fontSize: 15, color: '#FF8C00', textTransform: 'capitalize' }}>
                                    {dayjs(detailOuvert.date_planifiee).format('dddd DD MMMM YYYY')}
                                </div>
                                <div style={{ color: '#666', fontSize: 13, marginTop: 2 }}>
                                    {dayjs(detailOuvert.date_planifiee).format('HH:mm')}
                                    {detailOuvert.duree_estimee && ` — Durée estimée : ${detailOuvert.duree_estimee}h`}
                                </div>
                            </div>
                        </div>
                        <Descriptions column={1} bordered size="small">
                            <Descriptions.Item label="N° Intervention">
                                <span style={{ color: '#FF8C00', fontWeight: 700 }}>{detailOuvert.numero}</span>
                            </Descriptions.Item>
                            <Descriptions.Item label="Client">
                                <Space><UserOutlined />{detailOuvert.client_nom}</Space>
                            </Descriptions.Item>
                            ...
                            {detailOuvert.description && (
                                <Descriptions.Item label="Description">
                                    <span style={{ color: '#555', fontSize: 12 }}>{detailOuvert.description}</span>
                                </Descriptions.Item>
                            )}
                        </Descriptions>
                    </>
...
const Techniciens = () => {
    ...
    useEffect(() => { chargerTechniciens(); }, [filtreDisponible, filtreSpecialite]);

    const chargerTechniciens = async () => {
        setLoading(true);
        try {
            let url = '/techniciens/?';
            if (filtreDisponible !== '') url += `disponible=${filtreDisponible}&`;
    ...
    const modifierTechnicien = async (values) => {
        try {
            ...
            setModalModifier(false); chargerTechniciens();
        } catch { message.error('Erreur modification'); }
    };

    const supprimerTechnicien = async (id) => {
        try {
            await api.delete(`/techniciens/${id}/`);
            message.success('Technicien supprimé !'); chargerTechniciens();
        } catch { message.error('Erreur suppression'); }
    };

    const ouvrirModifier  = (t) => { setTechnicienSelectionne(t); formModifier.setFieldsValue(t); setModalModifier(true); };
    const ouvrirDetail    = (t) => { setTechnicienSelectionne(t); setDrawerDetail(true); };
    const ouvrirPlanning  = (t) => { setTechnicienSelectionne(t); setModalPlanning(true); };
        ...
        <Form form={form} layout="vertical" onFinish={onFinish} style={{ marginTop: 16 }}>
            <Space style={{ width: '100%' }} size={12}>
                <Form.Item label="Nom complet" name="nom" rules={[{ required: true, message: 'Nom obligatoire' }]} style={{ flex: 1 }}>
                    <Input placeholder="Ahmed Alami" prefix={<UserOutlined style={{ color: '#ccc' }} />} style={{ borderRadius: 8 }} />
                </Form.Item>
            ...
            <Space style={{ width: '100%' }} size={12}>
                ...
                <Form.Item label="Tarif horaire (MAD)" name="tarif_horaire" rules={[{ required: true, message: 'Tarif obligatoire' }]} style={{ flex: 1 }}>
                    <InputNumber min={0} placeholder="150" style={{ width: '100%', borderRadius: 8 }} addonAfter="MAD/h" />
                </Form.Item>
            </Space>
            <Form.Item label="Compétences" name="competences">
                <Input.TextArea rows={3} placeholder="Ex: Réparation PC, Installation Windows..." style={{ borderRadius: 8 }} />
            </Form.Item>
            <Form.Item label="Disponible" name="disponible" valuePropName="checked" initialValue={true}>
                <Switch checkedChildren="Disponible" unCheckedChildren="Indisponible" style={{ background: '#FF8C00' }} />
            </Form.Item>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 8 }}>
                <Button style={{ borderRadius: 8 }} onClick={() => { setModalCreer(false); setModalModifier(false); form.resetFields(); }}>Annuler</Button>
                <Button type="primary" htmlType="submit" style={{ background: '#FF8C00', borderColor: '#FF8C00', borderRadius: 8, fontWeight: 600 }}>Enregistrer</Button>
            </div>
        </Form>
    );
    ...
    const colonnes = [
        {
            title: 'Technicien', dataIndex: 'nom',
            render: (nom, record) => (
                <Space>
                    <Badge dot color={record.disponible ? '#52c41a' : '#f5222d'} offset={[-2, 32]}>
        ...
        },
        {
            ...
            render: (s) => <Tag color={couleurSpecialite[s]} style={{ borderRadius: 6 }}>{s?.toUpperCase()}</Tag>
        },
        { title: 'Téléphone', dataIndex: 'telephone', render: (tel) => tel || <span style={{ color: '#ccc' }}>N/A</span> },
        {
            title: 'Tarif/heure', dataIndex: 'tarif_horaire',
            render: (tarif) => <span style={{ color: '#FF8C00', fontWeight: 700 }}>{tarif} MAD</span>
        },
        {
            title: 'Compétences', dataIndex: 'competences',
            render: (comp) => comp
                ? <span style={{ color: '#666', fontSize: 12 }}>{comp.length > 40 ? comp.substring(0, 40) + '...' : comp}</span>
                : <span style={{ color: '#ccc' }}>Non renseigné</span>
        },
        {
            title: 'Disponibilité', dataIndex: 'disponible',
            render: (dispo) => dispo
                ? <Space><CheckCircleOutlined style={{ color: '#52c41a' }} /><span style={{ color: '#52c41a', fontWeight: 600 }}>Disponible</span></Space>
                : <Space><CloseCircleOutlined style={{ color: '#f5222d' }} /><span style={{ color: '#f5222d', fontWeight: 600 }}>Indisponible</span></Space>
        },
        {
                ...
                <Space>
                    <Tooltip title="Voir planning">
                        <Button type="text" icon={<CalendarOutlined />}
                            style={{ color: '#fff', background: '#FF8C00', borderRadius: 6, fontWeight: 600, fontSize: 12, height: 28, padding: '0 10px', display: 'flex', alignItems: 'center', gap: 4 }}
                            onClick={() => ouvrirPlanning(record)}>
                            Planning
                        </Button>
                    </Tooltip>
                    <Tooltip title="Voir détail">
                        <Button type="text" icon={<EyeOutlined />} style={{ color: '#FF8C00' }} onClick={() => ouvrirDetail(record)} />
                    </Tooltip>
                    <Tooltip title="Modifier">
                        <Button type="text" icon={<EditOutlined />} style={{ color: '#1890ff' }} onClick={() => ouvrirModifier(record)} />
                    </Tooltip>
                    <Tooltip title="Supprimer">
                        <Popconfirm title="Supprimer ce technicien ?" onConfirm={() => supprimerTechnicien(record.id)} okText="Oui" cancelText="Non" okButtonProps={{ danger: true }}>
    ...
    return (
        <div style={{ padding: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
                <div>
                    <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1A1A1A', margin: 0 }}>Techniciens</h1>
                    <p style={{ color: '#999', margin: '4px 0 0', fontSize: 14 }}>{techniciensFiltres.length} technicien(s)</p>
                </div>
                <Space>
                    <Button icon={<ReloadOutlined />} onClick={chargerTechniciens} style={{ borderRadius: 10 }}>Actualiser</Button>
                    <Button type="primary" icon={<PlusOutlined />} size="large" onClick={() => setModalCreer(true)}
                        style={{ background: '#FF8C00', borderColor: '#FF8C00', borderRadius: 10, fontWeight: 600, height: 44 }}>
                        Nouveau technicien
                    </Button>
                </Space>
            </div>

            <Card bordered={false} style={{ borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: 16 }}>
                <Space wrap>
                    <Input prefix={<SearchOutlined style={{ color: '#ccc' }} />}
                        placeholder="Rechercher par nom, spécialité ou téléphone..."
                        value={search} onChange={e => setSearch(e.target.value)}
                        allowClear style={{ width: 340, borderRadius: 8 }} />
                    <Select placeholder="Spécialité" allowClear style={{ width: 160 }} onChange={setFiltreSpecialite}>
                        {specialites.map(s => <Option key={s.key} value={s.key}>{s.label}</Option>)}
                    </Select>
                    <Select placeholder="Disponibilité" allowClear style={{ width: 160 }} onChange={setFiltreDisponible}>
            ...
            <Card bordered={false} style={{ borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                <Table columns={colonnes} dataSource={techniciensFiltres} rowKey="id"
                    loading={loading} pagination={{ pageSize: 10, showTotal: (t) => `${t} techniciens` }} />
            </Card>
            ...
            <Modal title={<span style={{ fontWeight: 700 }}>✏️ Modifier technicien</span>}
                open={modalModifier} onCancel={() => { setModalModifier(false); formModifier.resetFields(); }} footer={null} width={560}>
            ...
            <Drawer
                title={
                    <Space>
                        <Avatar style={{ background: '#FF8C00' }}>{technicienSelectionne?.nom?.charAt(0)?.toUpperCase()}</Avatar>
                        <span style={{ fontWeight: 700 }}>{technicienSelectionne?.nom}</span>
                    </Space>
                }
                open={drawerDetail} onClose={() => setDrawerDetail(false)} width={420}
                extra={
                    <Button icon={<CalendarOutlined />}
                        style={{ background: '#FF8C00', borderColor: '#FF8C00', color: '#fff', borderRadius: 8, fontWeight: 600 }}
                        onClick={() => { setDrawerDetail(false); setModalPlanning(true); }}>
                        Voir planning
                    </Button>
                }
            >
                {technicienSelectionne && (
                    <Descriptions column={1} bordered size="small">
                        <Descriptions.Item label="Nom">{technicienSelectionne.nom}</Descriptions.Item>
                        <Descriptions.Item label="Spécialité">
                            <Tag color={couleurSpecialite[technicienSelectionne.specialite]}>{technicienSelectionne.specialite?.toUpperCase()}</Tag>
                        </Descriptions.Item>
                        <Descriptions.Item label="Téléphone">{technicienSelectionne.telephone || 'N/A'}</Descriptions.Item>
                        <Descriptions.Item label="Tarif horaire">
                            <span style={{ color: '#FF8C00', fontWeight: 700 }}>{technicienSelectionne.tarif_horaire} MAD/h</span>
                        </Descriptions.Item>
                        <Descriptions.Item label="Compétences">{technicienSelectionne.competences || 'Non renseigné'}</Descriptions.Item>
                        <Descriptions.Item label="Disponibilité">{technicienSelectionne.disponible ? '✅ Disponible' : '❌ Indisponible'}</Descriptions.Item>
                    </Descriptions>
                )}
            </Drawer>

            <ModalPlanning technicien={technicienSelectionne} open={modalPlanning} onClose={() => setModalPlanning(false)} />
        </div>
    );
};
