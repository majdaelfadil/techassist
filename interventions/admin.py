...
class TechnicienAdmin(admin.ModelAdmin):
    list_display = ['nom', 'specialite', 'tarif_horaire', 'disponible']
    list_filter = ['specialite', 'disponible']

@admin.register(Appareil)
class AppareilAdmin(admin.ModelAdmin):
    list_display = ['marque', 'modele', 'type_appareil',
                    'client', 'sous_garantie']
    list_filter = ['type_appareil', 'sous_garantie']
    search_fields = ['marque', 'modele', 'numero_serie']

@admin.register(Piece)
class PieceAdmin(admin.ModelAdmin):
    list_display = ['nom', 'reference', 'quantite_stock',
                    'seuil_minimum', 'prix_unitaire']
    search_fields = ['nom', 'reference']

@admin.register(Intervention)
class InterventionAdmin(admin.ModelAdmin):
    list_display = ['numero', 'client', 'type_service',
                    'statut', 'urgence', 'technicien', 'date_creation']
    list_filter = ['statut', 'urgence', 'type_service', 'canal_entree']
    search_fields = ['numero', 'client__nom', 'description']

@admin.register(Facture)
class FactureAdmin(admin.ModelAdmin):
    list_display = ['numero', 'intervention', 'total_ttc',
...
class DiagnosticIAAdmin(admin.ModelAdmin):
    list_display = ['intervention', 'categorie_panne',
                    'complexite', 'duree_estimee', 'date_analyse']